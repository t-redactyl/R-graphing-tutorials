...
knitr::opts_chunk$set(message = F, error = F, warning = F, comment = NA, fig.align = 'center', dpi = 100, fig.width=6, fig.height=5, tidy = F, cache.path = '.cache/', fig.path = '3_Bar_Plots/')
```
...
```{r finalresult, echo=FALSE, cache=TRUE}
library(ggplot2)
library(ggthemes)
library(extrafont)
library(plyr)
...
p3 <- ggplot() + theme_fivethirtyeight() + scale_fill_fivethirtyeight() + 
  ...
  geom_text(data=charts.data, aes(x = year, y = pos, label = export), colour="white", size = 3.5,
            family = "Atlas Grotesk Medium", show.legend = F) + 
  scale_x_continuous(breaks=seq(2006,2014,1)) + 
  labs(x="Year", y="USD million") + 
  ggtitle("Composition of Exports to China ($)") +
