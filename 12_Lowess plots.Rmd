...
In order to initialise a plot we tell ggplot that `airquality` is our data, and specify that our x-axis plots the `Temp` variable and our y-axis plots the `Ozone` variable. We then instruct ggplot to render this as a LOWESS by adding the `stat_smooth()` option. Note that the default for `stat_smooth` is to include the confidence interval.
