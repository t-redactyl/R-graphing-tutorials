...
p12 <- ggplot(airquality, aes(x = Temp, y = Ozone)) + 
  ...
  geom_smooth(colour = fill, size = 1.5, alpha = 0.2, fill = fill) +
...
In order to initialise a plot we tell ggplot that `airquality` is our data, and specify that our x-axis plots the `Temp` variable and our y-axis plots the `Ozone` variable. We then instruct ggplot to render this as a LOWESS curve by adding the `stat_smooth()` option. Note that the default for `stat_smooth` is to include the confidence interval.
...
First, let's plot the linear model:
...
And have a look at the amount of variance it explains in ozone levels by extracting the adjusted $R^2$ from the linear regression model.
...
p12 <- p12 + ggtitle("Boxplot of mean ozone by month")
p12
```

# Changing the colour and size of the LOWESS curve

To change the colour of the LOWESS curve, we add a valid colour to the `colour` argument in `geom_smooth()` (note that we assigned this colour to variables outside of the plot to make it easier to change them). A list of valid colours is [here](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf).
...
p12 <- ggplot(airquality, aes(x = Temp, y = Ozone)) + 
  geom_point() + 
  geom_smooth(colour = fill, size = 1.5) +
  scale_x_continuous(name = "Temperature") +
  scale_y_continuous(name = "Mean ozone in\nparts per billion",
                     breaks = seq(0, 150, 25), limits=c(0, 150)) +
  ggtitle("Boxplot of mean ozone by month")
...
p12 <- ggplot(airquality, aes(x = Temp, y = Ozone)) + 
  geom_point() + 
  geom_smooth(colour = fill, size = 1.5, alpha = 0.2) +
  scale_x_continuous(name = "Temperature") +
  scale_y_continuous(name = "Mean ozone in\nparts per billion",
                     breaks = seq(0, 150, 25), limits=c(0, 150)) +
  ggtitle("Boxplot of mean ozone by month")
...
p12 <- ggplot(airquality, aes(x = Temp, y = Ozone)) + 
  geom_point() + 
  geom_smooth(colour = fill, size = 1.5, se = FALSE) +
  scale_x_continuous(name = "Temperature") +
  scale_y_continuous(name = "Mean ozone in\nparts per billion",
                     breaks = seq(0, 150, 25), limits=c(0, 150)) +
  ggtitle("Boxplot of mean ozone by month")
...
p12 <- ggplot(airquality, aes(x = Temp, y = Ozone)) + 
  ...
  geom_smooth(colour = "#56B4E9", size = 1.5, alpha = 0.2, fill = "#56B4E9") +
  scale_x_continuous(name = "Temperature") +
  scale_y_continuous(name = "Mean ozone in\nparts per billion",
                     breaks = seq(0, 150, 25), limits=c(0, 150)) +
  ggtitle("Boxplot of mean ozone by month") +
...
```{r install_officina_font, eval = FALSE}
library(sysfonts)
font.files()
font.add("OfficinaSanITC-Book", regular = "Officina Sans ITC Book.ttf")
font.families()
```
...
p12 <- ggplot(airquality, aes(x = Temp, y = Ozone)) + 
  ...
  geom_smooth(colour = "#4271AE", size = 1.5, alpha = 0.2, fill = "#4271AE") +
  scale_x_continuous(name = "Temperature") +
  scale_y_continuous(name = "Mean ozone in\nparts per billion",
                     breaks = seq(0, 150, 25), limits=c(0, 150)) +
  ggtitle("Boxplot of mean ozone by month") +
...
p12 <- ggplot(airquality, aes(x = Temp, y = Ozone)) + 
  ...
  geom_smooth(colour = "dodgerblue", size = 1.5, alpha = 0.2, fill = "dodgerblue") +
  scale_x_continuous(name = "Temperature") +
  scale_y_continuous(name = "Mean ozone in\nparts per billion",
                     breaks = seq(0, 150, 25), limits=c(0, 150)) +
  ggtitle("Boxplot of mean ozone by month") +
  theme_fivethirtyeight() + scale_fill_fivethirtyeight() +   
  theme(axis.title = element_text(family="AtlasGrotesk-Light"),
    legend.position="bottom", 
    legend.direction="horizontal",
    legend.box = "horizontal", 
    legend.title=element_text(family="AtlasGrotesk-Light", size = 10),
    legend.text=element_text(family="AtlasGrotesk-Light", size = 10),
    plot.title=element_text(family="AtlasGrotesk-Medium"), 
    text=element_text(family="AtlasGrotesk-Light")) 
p12
```
