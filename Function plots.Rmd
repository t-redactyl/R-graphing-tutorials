...
```{r function_5}
p1 <- p1 + scale_x_continuous(name = "Probability") +
        scale_y_continuous(name = "Frequency")
p1
```
...
```{r function_7}
p1 <- p1 + ggtitle("Normal function curves of probabilities")
p1
```
...
p1 <- p1 + stat_function(fun=funcShaded, geom="area", fill="#84CA72", alpha=0.2)
p1
```
